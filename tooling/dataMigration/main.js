/*
ARCHIVED - this is the one-time v1 (JSON files) -> v2 (per-guild SQLite)
migration that already ran historically. It imports guild.js, which is now
the v3 (Postgres) service with a different constructor signature
(guildId, database) - so this script will not run as-is anymore. Left here
for reference only. If you have legacy JSON data that was never migrated,
either restore an older commit of guild.js/sqlite.js to run this first, or
adapt it to write to Postgres directly. For migrating an existing v2
per-guild SQLite database (the common case), use
tooling/dataMigration/sqliteToPostgres.js instead.
*/
const fs = require('fs');
